...
class Q368 {
    ...
    public List<Integer> largestDivisibleSubset(int[] nums) {
        ...
        return largestDivisibleSubsetHelper(nums, 0, lookup);
    }

    private List<Integer> largestDivisibleSubsetHelper(int[] nums, int index, Map<Integer, List<Integer>> lookup) {
        if(lookup.containsKey(index)) {
            return lookup.get(index);
        }

        List<Integer> maxResult = new ArrayList<>();
        int divisor = index == 0 ? 1 : nums[index-1];
        for(int i=index; i<nums.length; i++) {
            if(nums[i]%divisor==0) {
                List<Integer> result = new ArrayList<>();
                result.add(nums[i]);
                result.addAll(largestDivisibleSubsetHelper(nums, i+1, lookup));
                if(result.size()>maxResult.size()) {
                    maxResult = new ArrayList<>(result);
                }
            }
        }

        lookup.put(index, maxResult);
        return maxResult;
    }
    ...
    public List<Integer> largestDivisibleSubset1(int[] nums) {
        ...
        largestDivisibleSubsetHelper1(nums, 0, new ArrayList<Integer>(), result);
        return result;
    }

    private void largestDivisibleSubsetHelper1(int[] nums, int index, List<Integer> subset, List<Integer> result) {
        for(int i=index; i<nums.length; i++) {
            if(compatible1(subset, nums[i])) {
                subset.add(nums[i]);
                largestDivisibleSubsetHelper1(nums, i+1, subset, result);
    ...
    private boolean compatible1(List<Integer> subset, int num) {
        return subset.isEmpty()||num%subset.get(subset.size()-1)==0;
    }
    ...
    public List<Integer> largestDivisibleSubset0(int[] nums) {
        ...
        largestDivisibleSubsetHelper0(nums, 0, new ArrayList<Integer>(), result);
        return result;
    }

    private void largestDivisibleSubsetHelper0(int[] nums, int index, List<Integer> subset, List<Integer> result) {
        if(index==nums.length) {
            if(subset.size()>result.size()) {
                //result = new ArrayList<Integer>(subset); //Updating result content using this approach does not work. Why?
                result.clear();
                for(Integer num : subset) {
                    result.add(num);
                }
        ...
        for(int i=index; i<nums.length; i++) {
            if(compatible0(subset, nums[i])) {
                subset.add(nums[i]);
                largestDivisibleSubsetHelper0(nums, i+1, subset, result);
                subset.remove(subset.size()-1);
            }
        }
    ...
    private boolean compatible0(List<Integer> subset, int num) {
        for(int i=0; i<subset.size(); i++) {
            int cur = subset.get(i);
            if(cur%num!=0&&num%cur!=0) {
                return false;
            }
        }
